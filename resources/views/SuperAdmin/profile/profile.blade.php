...
<div class="container mt-5">
    <div class="profile-card">
        ...
        <div class="profile-body">
            <form action="{{ route('update.profile', $superadmin->id) }}" method="post" enctype="multipart/form-data">

                <div class="profile-header">
                    <input type="file" id="profileImageInput" name="image" accept="image/*" style="display:none;">
                    <img id="profileImagePreview"
                        src="{{ $superadmin->image && file_exists(public_path('storage/'.$superadmin->image)) 
            ? asset('storage/'.$superadmin->image) 
            : asset('images/user.jpeg') }}"
                        onclick="document.getElementById('profileImageInput').click();">

                    <h4 class="mt-3">{{ $superadmin->name }}</h4>
                    <p class="text-light mb-0">{{ $superadmin->email }}</p>
                    <small>{{ __('Students_trans.SuperAdmin') }}</small>
                </div>
                @csrf
                <div class="row mb-3">
                    <div class="col-md-4">
                    ...
                    <div class="col-md-8">
                        <input type="text" name="name_ar"
                            value="{{ $superadmin->getTranslation('name', 'ar') }}"
                            class="form-control">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-8">
                        <input type="text" name="name_en"
                            value="{{ $superadmin->getTranslation('name', 'en') }}"
                            class="form-control">
